...
def Test(config, vars):
	...
	# Test code here...
	#pushSeqCmd(dt, 'scriptFunction', 'TestFunction')

	pushSeqCmd(dt, 'CNPY_LK_N_LOW_LEV', 0)
	pushSeqCmd(2, 'CNPY_LK_N_LOW_LEV', 1)
	pushSeqCmd(2, 'CNPY_LK_N_LOW_LEV', 2)
	pushSeqCmd(2, 'CNPY_LK_N_LOW_LEV', 1)
	pushSeqCmd(2, 'CNPY_LK_N_LOW_LEV', 0)

	#pushSeqCmd(dt, 'CNPY_LK_N_LOW_LEV', 2)
	#pushSeqCmd(1, 'CNPY_LK_N_LOW_LEV', 0)
	#pushSeqCmd(dt, 'CNPY_REST', 1)
	#pushSeqCmd(dt, 'CNPY_HAND1', -1)
	#pushSeqCmd(dt, 'CNPY_HAND2', 1)
...
def TestFunction(dcsBiosExport):
	seq = []
	dt = 0.3
	...
	latDir = dcsBiosExport.getControlState('CommonData/LAT_DIR') # N or S
	latDeg = dcsBiosExport.getControlState('CommonData/LAT_DEG')
	latMin = dcsBiosExport.getControlState('CommonData/LAT_SEC') # Control is labeled LAT_SEC, but it's actually minutes.
	latMinFrac = dcsBiosExport.getControlState('CommonData/LAT_SEC_FRAC') # Control is labeled LAT_SEC_FRAC, but it's actually fractional minutes.
	longDir = dcsBiosExport.getControlState('CommonData/LON_DIR') # E or W
	longDeg = dcsBiosExport.getControlState('CommonData/LON_DEG')
	longMin = dcsBiosExport.getControlState('CommonData/LON_SEC') # Control is labeled LON_SEC, but it's actually minutes.
	longMinFrac = dcsBiosExport.getControlState('CommonData/LON_SEC_FRAC') # Control is labeled LON_SEC_FRAC, but it's actually fractional minutes.

	#pushSeqCmd(dt, 'scriptCockpitState', control=[
	#	['M-2000C/PCN_DISP_L_1_0', 0],
	#	['M-2000C/PCN_DISP_L_1_1', 2],
	#	['M-2000C/PCN_DISP_L_1_2', 0],
	#	['M-2000C/PCN_DISP_L_1_3', 2],
	#	['M-2000C/PCN_DISP_L_1_4', 2],
	#	['M-2000C/PCN_DISP_L_1_5', 0],
	#	['M-2000C/PCN_DISP_L_1_6', 2],
	#	['M-2000C/PCN_DISP_L_1_7', 0],
